  ...
  const technicalRisk =
    telecom && 'risk' in telecom
      ? {
          ...
          lineStatus:
            'lineStatus' in telecom ? telecom.lineStatus ?? null : null,
          isVoip:
            'isVoip' in telecom ? telecom.isVoip ?? null : null,
        }
      : {
          level: null,
          disposable: null,
  ...
  try {
    ...
    if (phoneRow?.id) {
      const { data: rep } = await supabase
        ...
        .select(
          'community_score,total_reports,total_positive,total_negative,dominant_category'
        )
  ...
  if (authorization.startsWith('Bearer ')) {
    ...
    try {
      ...
      if (userError || !userData.user) {
        console.error(
          'Histórico não registrado: sessão não validada.',
          userError
        );
      } else {
        const metadata = {
          ...
          line_type: phone.lineType ?? null,
          carrier_current: telecom?.carrierCurrent ?? null,
        };

        const { data: historyId, error: historyError } =
          await authSupabase.rpc('record_my_lookup', {
            p_e164: phone.e164,
            p_metadata: metadata,
          });
        ...
        if (historyError) {
          console.error(
            'Falha ao registrar histórico autenticado:',
            historyError
          );
        } else {
          historyRecorded = Boolean(historyId);
        }
      }
    } catch (error) {
      console.error(
        'Falha inesperada ao registrar histórico autenticado:',
        error
      );
    }
  }
